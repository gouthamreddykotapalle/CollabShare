...
def getAccessIds(userId):
    ...
    print(docIdListofLists)
    docIds = flat_list = [docId for sublist in docIdListofLists for docId in sublist]
    print(docIds)
    if not docIds:
        return [], []
    attributes_to_get = ['pageIds']
    pageIds = scan_dynamo(docIds, "documents", attributes_to_get, "documentId")
    print(pageIds)
